...
window.Echo.channel('blogs').listen('.blog.created', (event) => {
    ...
    //console.log(loggedInUserIdNumber);
    const eventUserId = event.user_id ? Number(event.user_id) : null
    ...
    if (blogList) {
        ...
        setTimeout(() => {
            newBlog.classList.remove("side-in");
        }, 500)
    } if (event.title && event.user_name) {
        ...
        notification.innerHTML = `
            <span>📝 New Blog:</span> <span>${event.title} by ${event.user_name}</span>
        `;
...
window.Echo.channel('blogs').listen('.blog.deleted', (event) => {
    console.log('New Blog Deleted:', event.blogId);
